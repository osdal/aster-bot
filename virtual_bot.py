...
SYMBOLS = os.getenv("LIVE_SYMBOL", "SOLUSDT").split(",")
...
DATA_DIR = "data"
CSV_FILE = os.path.join(DATA_DIR, "trades_multi.csv")
...
def safe_get_price(symbol):
    ...
    data = public_get("/fapi/v1/ticker/price")

    if not data:
        return None

    try:
        return float([x for x in data if x["symbol"] == symbol][0]["price"])
    except:
        return None
...
def get_trend_and_slope(closes):
...
print("=== MULTI-VIRTUAL BOT STARTED ===")
...
while True:
    ...
    for sym in SYMBOLS:
        ...
        price, closes, highs, lows = data

        atr = get_atr(highs, lows, closes)

        volatility = atr / price

        trend, slope = get_trend_and_slope(closes)

        slope_ratio = abs(slope) / price


        msg = f"{datetime.now()} | {sym}: PRICE={price:.4f}, ATR={atr:.6f}, VOL={volatility:.6f}, SLOPE={slope_ratio:.8f}, TREND={trend}"
        ...
        if ENABLE_COOLDOWN and not TEST_MODE and now - last_trade_time[sym] < COOLDOWN_SEC:
            ...
            print(f"{sym}: SIGNAL BLOCKED -> cooldown active")
        ...
        while True:
            ...
            if hit:
                ...
                record_trade(sym, side, entry, exit_price, result, pnl, equity,
                             qty, atr, volatility, slope_ratio, tp, sl, start)
